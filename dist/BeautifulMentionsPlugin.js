...
import { $createTextNode, $getSelection, $nodesOfType, $setSelection, BLUR_COMMAND, COMMAND_PRIORITY_LOW, KEY_BACKSPACE_COMMAND, KEY_DOWN_COMMAND, KEY_SPACE_COMMAND, } from "lexical";
...
export function BeautifulMentionsPlugin(props) {
    ...
    const convertTextToMention = useCallback(() => {
        ...
        const mentionNode = $createBeautifulMentionNode(trigger, option.value, option.data);
    ...
    React.useEffect(() => {
        return mergeRegister(editor.registerCommand(KEY_DOWN_COMMAND, handleKeyDown, COMMAND_PRIORITY_LOW), editor.registerCommand(KEY_BACKSPACE_COMMAND, handleDeleteMention, COMMAND_PRIORITY_LOW), editor.registerCommand(BLUR_COMMAND, () => {
            if (insertOnBlur) {
        ...
        }, COMMAND_PRIORITY_LOW), editor.registerCommand(INSERT_MENTION_COMMAND, ({ trigger, value, focus = true }) => {
            setSelection();
            const inserted = $insertMentionAtSelection(triggers, punctuation, trigger, value);
            if (!focus) {
                archiveSelection();
            }
            return inserted;
        }, COMMAND_PRIORITY_LOW), editor.registerCommand(REMOVE_MENTIONS_COMMAND, ({ trigger, value, focus }) => {
            const removed = $removeMention(trigger, value, focus);
            if (removed && !focus) {
                archiveSelection();
            }
            return removed;
        }, COMMAND_PRIORITY_LOW), editor.registerCommand(RENAME_MENTIONS_COMMAND, ({ trigger, newValue, value, focus }) => {
            const renamed = $renameMention(trigger, newValue, value, focus);
            if (renamed && !focus) {
                archiveSelection();
            }
            return renamed;
        }, COMMAND_PRIORITY_LOW), editor.registerCommand(OPEN_MENTION_MENU_COMMAND, ({ trigger }) => {
            setSelection();
