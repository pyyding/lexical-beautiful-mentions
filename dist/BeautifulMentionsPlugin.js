...
import { $createTextNode, $getSelection, $isNodeSelection, $nodesOfType, $setSelection, BLUR_COMMAND, COMMAND_PRIORITY_LOW, COMMAND_PRIORITY_NORMAL, KEY_BACKSPACE_COMMAND, KEY_DOWN_COMMAND, KEY_SPACE_COMMAND, PASTE_COMMAND, SELECTION_CHANGE_COMMAND, } from "lexical";
...
import { $getSelectionInfo, $selectEnd, DEFAULT_PUNCTUATION, LENGTH_LIMIT, PRE_TRIGGER_CHARS, TRIGGERS, VALID_CHARS, getCreatableProp, getMenuItemLimitProp, getTextContent, getTriggerRegExp, isWordChar, } from "./mention-utils";
...
export function BeautifulMentionsPlugin(props) {
    ...
    const [triggerRegExp, setTriggerRegExp] = useState(null);
    ...
    const handleSelectOption = useCallback((selectedOption, nodeToReplace, closeMenu) => {
        editor.update(() => {
            ...
            const mentionNode = $createBeautifulMentionNode(trigger, triggerRegExp, value, selectedOption.data);
            if (nodeToReplace) {
    ...
    const handleSelectMenuItem = useCallback((selectedOption, nodeToReplace, closeMenu) => {
        if (!trigger) {
            return;
        }
    ...
    const convertTextToMention = useCallback(() => {
        ...
        const mentionNode = $createBeautifulMentionNode(trigger, triggerRegExp, option.value, option.data);
        editor.update(() => {
    ...
    const handleDeleteMention = useCallback((event) => {
    ...
    const insertSpaceIfNecessary = useCallback((startsWithTriggerChar = false) => {
        const selectionInfo = $getSelectionInfo(triggers, punctuation);
        if (!selectionInfo) {
            return false;
        }
    ...
    useEffect(() => {
        ...
        return mergeRegister(editor.registerCommand(SELECTION_CHANGE_COMMAND, () => {
            ...
            return false;
        }, COMMAND_PRIORITY_LOW), editor.registerCommand(KEY_DOWN_COMMAND, handleKeyDown, COMMAND_PRIORITY_LOW), editor.registerCommand(KEY_BACKSPACE_COMMAND, handleDeleteMention, COMMAND_PRIORITY_LOW), editor.registerCommand(BLUR_COMMAND, () => {
            if (insertOnBlur) {
        ...
        }, COMMAND_PRIORITY_LOW), editor.registerCommand(INSERT_MENTION_COMMAND, ({ trigger, value, data, focus = true }) => {
            restoreSelection();
            ...
            return inserted;
        }, COMMAND_PRIORITY_LOW), editor.registerCommand(REMOVE_MENTIONS_COMMAND, ({ trigger, value, focus }) => $removeMention(trigger, value, focus), COMMAND_PRIORITY_LOW), editor.registerCommand(RENAME_MENTIONS_COMMAND, ({ trigger, newValue, value, focus }) => $renameMention(trigger, newValue, value, focus), COMMAND_PRIORITY_LOW), editor.registerCommand(OPEN_MENTION_MENU_COMMAND, ({ trigger }) => {
            restoreSelection();
            return $insertTriggerAtSelection(triggers, punctuation, trigger);
        }, COMMAND_PRIORITY_LOW), editor.registerCommand(PASTE_COMMAND, handlePaste, COMMAND_PRIORITY_LOW));
    }, [
        editor,
        triggers,
        punctuation,
    ...
    return (_jsx(LexicalTypeaheadMenuPlugin, { commandPriority: COMMAND_PRIORITY_NORMAL, onQueryChange: setQueryString, onSelectOption: handleSelectMenuItem, triggerFn: checkForMentionMatch, options: options, anchorClassName: menuAnchorClassName, onClose: handleClose, menuRenderFn: (anchorElementRef, { selectedIndex, selectOptionAndCleanUp, setHighlightedIndex }) => {
            ...
            if (anchorElementRef.current &&
                ...
                isEditorFocused &&
                EmptyComponent) {
                return ReactDOM.createPortal(_jsx(EmptyComponent, {}), anchorElementRef.current);
            }
            return anchorElementRef.current && open
                ? ReactDOM.createPortal(_jsx(MenuComponent, { loading: loading, role: "menu", "aria-label": "Choose a mention", "aria-hidden": !open, "aria-activedescendant": !IS_MOBILE &&
                        selectedIndex !== null &&
                        !!options[selectedIndex]
                        ? options[selectedIndex].displayValue
                        : "", children: options.map((option, i) => (_jsx(MenuItemComponent, Object.assign({ tabIndex: -1, selected: !IS_MOBILE && selectedIndex === i, ref: option.setRefElement, role: "menuitem", "aria-selected": !IS_MOBILE && selectedIndex === i, "aria-label": `Choose ${option.value}`, item: option.menuItem, itemValue: option.value, label: option.displayValue }, option.data, { onClick: () => {
